...
namespace N64
{
    ...
    public class ImageRGBA32
    {
        ...
        public ImageRGBA32(int width, int height, byte[] buffer)
        {
            if (width * height * 4 != buffer.Length)
                throw new Exception($"buffer has length {buffer.Length}, which does not match dimensions {width}x{height}");
            Width = width;
            Height = height;
            Buffer = buffer;
        }
    ...
    public static class N64Texture
    {
        ...
        public static byte[] EncodeBitmap(ImageRGBA32 bmp, N64TexFormat format)
        {
            var a = ConvertFormat(format);
            return EncodeBitmap(bmp, a.Item1, a.Item2);
        }
        public static byte[] EncodeBitmap(ImageRGBA32 bmp, G_IM_FMT fmt, G_IM_SIZ siz)
        {
