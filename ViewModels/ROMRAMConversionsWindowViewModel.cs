...
public partial class ROMRAMConversionsWindowViewModel : ObservableObject
{
    ...
    public enum InputAddressTypeEnum { ROM, VROM, VRAM }; // keep in sync with the ComboBoxItem elements in the view axaml
    [ObservableProperty]
        ...
        PropertyChanged += (sender, e) =>
        {
            switch (e.PropertyName)
            {
                case nameof(InputAddressTypeIndex):
                    Debug.Assert(InputAddressTypeIndex >= 0 && InputAddressTypeIndex < 3);
                    InputAddressType = Enum.GetValues<InputAddressTypeEnum>()[InputAddressTypeIndex];
                    break;
                case nameof(InputAddressStr):
                    ...
                    uint inAddr;
                    if (uint.TryParse(inAddrStr, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out inAddr))
                        InputAddress = inAddr;
                    else
                        InputAddress = null;
                    break;
    ...
    {
        Debug.Assert(InputAddress != null);
