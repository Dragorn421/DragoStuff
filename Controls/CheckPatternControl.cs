...
{
    public static readonly StyledProperty<int> CheckerSizeProperty =
        AvaloniaProperty.Register<DLViewerControl, int>(nameof(CheckerSize), defaultValue: 10);
    ...
    private Dictionary<ThemeVariant, (Color, Color)> _checkColors = new()
    {
        {
            ThemeVariant.Dark,
            (Color.FromRgb(50, 50, 50), Color.FromRgb(0, 0, 0))
        },
        {
            ThemeVariant.Light,
            (Color.FromRgb(255, 255, 255), Color.FromRgb(200, 200, 200))
        },
    };
    ...
    private void UpdateCheckPatternTileBrush()
    {
        ...
        {
            Debug.Assert(_checkColors.ContainsKey(ThemeVariant.Light));
            themeVariant = ThemeVariant.Light;
        }
        var (evenColor, oddColor) = _checkColors[themeVariant];

        int repeatsX = 10, repeatsY = 10; // purely for optimization so ImageBrush has less repeats to do
        var bitmap = new RenderTargetBitmap(new PixelSize(CheckerSize * 2 * repeatsX, CheckerSize * 2 * repeatsY));
        using (var dc = bitmap.CreateDrawingContext())
        {
            ...
            for (int i = 0; i < repeatsX; i++)
            {
                ...
                for (int j = 0; j < repeatsY; j++)
                {
                    ...
                    dc.FillRectangle(new SolidColorBrush(evenColor), rect.WithX(x + 0).WithY(y + 0));
                    dc.FillRectangle(new SolidColorBrush(evenColor), rect.WithX(x + CheckerSize).WithY(y + CheckerSize));
                }
            }
        }
