...
public partial class HexViewerControl : UserControl
{
    ...
    public static readonly StyledProperty<byte[]?> DataBytesProperty =
        AvaloniaProperty.Register<DLViewerControl, byte[]?>(nameof(DataBytes), defaultValue: null);
        ...
        PropertyChanged += (sender, e) =>
        {
            switch (e.Property.Name)
            {
                ...
                case nameof(FirstByteAddress):
                    ...
                    uint lastRowAddress = data.Length == 0 ? firstRowAddress : (firstByteAddress + (uint)data.Length - 1) / BYTES_PER_ROW * BYTES_PER_ROW;
                    ...
                    headerText[0] = '_';
                    Debug.Assert(BYTES_PER_ROW <= 0xFF);
                    for (int i = 0; i < BYTES_PER_ROW; i++)
                    {
                        headerText.AppendFormat(" {0:X2}", i);
                    }

                    for (uint rowAddress = firstRowAddress; rowAddress <= lastRowAddress; rowAddress += BYTES_PER_ROW)
                    {
