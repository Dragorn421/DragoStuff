...
public partial class CameraHandling : ObservableObject
{
    ...
    {
        _camPos.Z += delta * 400 * Math.Max(0.01f, Math.Abs(_camPos.Z) / 10000) * MOVE_SENSITIVITY_Z;
    ...
    public void OnMouseMoveWithRightClickHeld(Vector2 pos)
    {
        if (_oldPos != null)
        {
            ...
            _camPos.X += (pos.X - oldPos.X) * Math.Max(0.01f, Math.Abs(_camPos.Z) / 1000) * MOVE_SENSITIVITY_X;
            _camPos.Y -= (pos.Y - oldPos.Y) * Math.Max(0.01f, Math.Abs(_camPos.Z) / 1000) * MOVE_SENSITIVITY_Y;
